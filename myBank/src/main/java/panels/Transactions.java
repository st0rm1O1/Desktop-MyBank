...
public class Transactions extends JPanel {
	...
	private JPanel btn_icon, empty_panel;
	private JLabel back_btn, bal_label, bal_txt, empty_label, head_label, op_label;
	private JTable transaction_table;
	private JScrollPane table_scroll;
	private JButton btn_OP, btn_M, op_btn, close_btn;
	private JPanel crud_panel;
	private JTextField op_field;
	private JPopupMenu menu;
	private JMenuItem deposit, withdraw, add_user, update_user, view_transaction, delete_user;
	...
	public Transactions(LayerPanel layerPanel, UserModel userModel) {
		...
		menu = new JPopupMenu();
		deposit = new JMenuItem("Deposit", new ImageIcon(getClass().getResource(res.getDEPOSIT_PATH())));
		withdraw = new JMenuItem("Withdraw", new ImageIcon(getClass().getResource(res.getWITHDRAW_PATH())));
		add_user = new JMenuItem("Add New User", new ImageIcon(getClass().getResource(res.getADD_USER_PATH())));
		update_user = new JMenuItem("Update User", new ImageIcon(getClass().getResource(res.getUPDATE_USER_PATH())));
		delete_user = new JMenuItem("Delete User", new ImageIcon(getClass().getResource(res.getDELETE_USER_PATH())));
		view_transaction = new JMenuItem("View Transactions", new ImageIcon(getClass().getResource(res.getVIEW_USER_PATH())));
...
//		setUI(new ImageRender("/MESH/gradient_"+ new Random().nextInt(12) + ".png"));
		...
		deposit.setFont(res.getInter(1, 16));
		withdraw.setFont(res.getInter(1, 16));
		add_user.setFont(res.getInter(1, 16));
		update_user.setFont(res.getInter(1, 16));
		delete_user.setFont(res.getInter(1, 16));
		view_transaction.setFont(res.getInter(1, 16));
...
//					dialog.displayDialog(Transactions.this, 400, "INSUFFICIENT BALANCE!", 0);
		...
		view_transaction.addActionListener(new ActionListener() {
		...
		transaction_table.getTableHeader().setFont(res.getInter(3, 20));
		transaction_table.setFont(res.getInter(1, 18));
		...
		transaction_table.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (op_btn.getText().toString().equals("UPDATE")) {
				...
				if (op_btn.getText().toString().equals("DELETE")) {
				...
				if (op_btn.getText().toString().equals("INSERT")) {
				...
				if (op_btn.getText().toString().equals("OPEN")) {
		...
		bal_label.setFont(res.getInter(2, 16));
		...
		bal_txt.setFont(res.getInter(3, 38));
		bal_txt.setBounds(460, 80, 510, 60);
		
		
		btn_OP.setForeground(Color.WHITE);
		btn_OP.setFont(res.getInter(2, 22));
		...
		btn_M.setFont(res.getInter(2, 22));
		...
		head_label.setFont(res.getInter(3, 56));
		
		empty_label.setHorizontalAlignment(SwingConstants.CENTER);
		empty_label.setFont(res.getInter(2, 24));
		...
		op_label.setFont(res.getInter(3, 38));
		...
		op_field.setFont(res.getInter(1, 20));
		
		op_btn.setForeground(Color.WHITE);
		op_btn.setFont(res.getInter(2, 20));
		...
		op_btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row_index == -1) 
					...
					switch(op_btn.getText().toString()) {
						...
						case "UPDATE": {
							...
							if (!db.updateUser(op_field.getText().toString().replace("\u20B9", ""),
									col_index, 
		...
		close_btn.setFont(res.getInter(2, 20));
