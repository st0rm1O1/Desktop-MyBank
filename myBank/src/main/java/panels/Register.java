...
//	private final String emailValidationPattern = "^(.+)@(\\S+)$";
	...
	private JTextArea public_msg, mail_msg, private_msg, confirm_msg, dob_msg, contact_msg, gender_msg;
	private JPanel btn_icon, private_icon, confirm_icon, private_icon_border, confirm_icon_border;
	private JLabel back_btn, register_txt, register_action, public_util, private_util, confirm_util, mail_util, contact_util, gender_util, dob_util;
	private JTextField public_key, email_key, dob_key, contact_key;
	private JPasswordField private_key, confirm_key;
	private JButton btn;
	private JComboBox<enuModel> gender_key;
	private DAO db;
	...
	public Register(LayerPanel layerPanel, UserModel userModel) {
		...
		gender_key = new JComboBox<enuModel>(new DefaultComboBoxModel<enuModel>(enuModel.values()));
...
//		setUI(new ImageRender("/MESH/gradient_"+ new Random().nextInt(12) + ".png"));
		...
		private_msg.setFont(res.getInter(1, 16));
		...
		confirm_msg.setFont(res.getInter(1, 16));
		...
		dob_msg.setFont(res.getInter(1, 16));
		...
		contact_msg.setFont(res.getInter(1, 16));
		...
		gender_msg.setFont(res.getInter(1, 16));
		...
		public_msg.setFont(res.getInter(1, 16));
		...
		mail_msg.setFont(res.getInter(1, 16));
		...
		public_key.setFont(res.getInter(1, 22));
		...
		private_key.setFont(res.getInter(1, 22));
		...
		private_key.addFocusListener(new FocusListener() {
		    ...
		    public void focusGained(FocusEvent e) {
		        if (String.valueOf(private_key.getPassword()).equals(private_hint)) {
					...
					layerPanel.repaint();
					private_icon.setUI(new ImageRender(res.getLOCK_PATH()));
		        }
		    }
		...
		confirm_key.setFont(res.getInter(1, 22));
		...
		confirm_key.addFocusListener(new FocusListener() {
		    ...
		    public void focusGained(FocusEvent e) {
		        if (String.valueOf(confirm_key.getPassword()).equals(confirm_hint)) {
					...
					layerPanel.repaint();
					confirm_icon.setUI(new ImageRender(res.getLOCK_PATH()));
		        }
		    }
		...
		email_key.setFont(res.getInter(1, 22));
		...
		dob_key.setFont(res.getInter(1, 22));
		...
		contact_key.setFont(res.getInter(1, 22));
		...
		gender_key.setFont(res.getInter(1, 22));
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String user_data = public_key.getText().toString().trim();
				String email_data = email_key.getText().toString().trim();
				String dob_data = dob_key.getText().toString().trim();
				int gender_data = gender_key.getSelectedIndex();
				String contact_data = contact_key.getText().toString().trim();
		...
		private_icon_border.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!String.valueOf(private_key.getPassword()).equalsIgnoreCase(private_hint)) {
					...
					else if (!private_visibility) {
						private_visibility = true;
						private_key.setEchoChar((char) 0);
						layerPanel.repaint();
						private_icon.setUI(new ImageRender(res.getUNLOCK_PATH()));
					}
		...
		confirm_icon_border.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!String.valueOf(confirm_key.getPassword()).equalsIgnoreCase(confirm_hint)) {
					...
					else if (!confirm_visibility) {
						confirm_visibility = true;
						confirm_key.setEchoChar((char) 0);
						layerPanel.repaint();
						confirm_icon.setUI(new ImageRender(res.getUNLOCK_PATH()));
					}
